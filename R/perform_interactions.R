perform_interactions <- function(pop, sub_pop, logDir, params) {
  ...
  for (snap in 1:params[["nrOfSnapshots"]]) {
    ...
    for (i in 1:params[["interactionsPerSnapshot"]]) {
      perform_single_interaction(pop, sub_pop, interactionsLog, snap, groupsInfo, params)
    }
    save_population(pop, extraCols = base::list(snapshot = snap), logDir = logDir)
    save_sub_population(sub_pop, extraCols = base::list(snapshot = snap), logDir = logDir) ### this is an adjustment of mine
