...
- `P1` etc.: all numeric columns starting with `P` are the acoustic [features](input-data). The function `get_Pcols()` returns the names of the feature columns and `get_N_Pcols()` returns the amount of feature columns, given a data frame like the input data or population as an argument.
...
- `valid`: logical column indicating whether the corresponding row in the data frame is valid. If `FALSE`, this indicates that the given exemplar has been removed from the agent's memory and is no longer of influence to the simulation.
...
- `timeStamp`: numeric column indicating how old the given exemplar is; time stamps are created and updated within word class, with more recent exemplars receiving higher time stamps.
...
- `simulationNr`: same as `snapshot`
- `valid`: logical column; always `TRUE`
...
- `run`: character column indicating the run
- `snapshot`: character column indicating the snapshot
- `Agent`: numeric column indicating the ID of the agent
- `name`: character column with one of `nFeatures` (number of acoustic features), `qda` (quadrativ discriminatory analysis for [posterior probability](perception-1) memorisation criteria), `GMM` ([Gaussian Mixture Model](phonology) if `useFlexiblePhonology` is `TRUE`), `nAccepted` (overall number of tokens that the agent has accepted and memorised), `nForgotten` (over number of exemplars that the agent has removed from memory)
- `value`: list of values corresponding to the cached object indicated by `name`
- `valid`: logical column indicating whether the cached object is valid
