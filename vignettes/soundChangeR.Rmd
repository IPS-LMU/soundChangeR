...
During a simulation with `soundChangeR`, the agents exchange tokens for a given amount of interactions. An interaction always consists of an agent speaker who produces a new token of a word and an agent listener who decides whether or not to memorise the token. Agents can interact with each other either freely or they do so only within or only across predefined groups (such as older vs. younger, region A vs. region B vs. region C, or similarly meaningful groups). 
...
Say, `albr` is the agent speaker and has chosen to produce a token of the word *food*. Then `albr` gathers all exemplars of *food* in its memory (as shown by the `food` labels in the plots below) and estimates a Gaussian model over them (as exemplified by the ellipses) in the three-dimensional DCT space (here broken down into two 2D plots for reasons of legibility). A new token of *food*, shown in orange, is then sampled from the Gaussian distribution:
...
The second issue (i.e. too many exemplars) can be avoided by having the agent listeners forget exemplars, i.e. remove an exemplar from memory that belongs to the same word class as the memorised token. The only restriction on forgetting is that word classes cannot contain less tokens than determined by the simulation setting `minTokens`, so if the deletion of an exemplar would leave the word class with less exemplars than `minTokens`, forgetting is blocked.

Let's assume that `elwi` has decided to memorise the perceived token of *food* in the example above. Depending on the model's settings, a random exemplar of *food* can be removed unless the result is that there are less than 10 exemplars of *food* left afterwards (because the default simulation setting is `minTokens = 10`).
...
## Data Structures and Analysis of Results
...
Cronenberg, J. (2024): [A Cognitive and Computational Model of the Sound Change from Pre- to Post-Aspiration in Andalusian Spanish.](https://edoc.ub.uni-muenchen.de/33026/1/Cronenberg_Johanna.pdf) Dissertation, LMU Munich.

Gubian, M., Cronenberg, J., and Harrington, J. (2023): [Phonetic and phonological sound changes in an agent-based model.](https://www.sciencedirect.com/science/article/pii/S0167639323000122?ssrnid=4063582&dgcid=SSRN_redirect_SD) Speech Communication 147, pp. 93-115.
