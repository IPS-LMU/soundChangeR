...
test_that("exemplar2matrix produces expected vector and matrix", {
  expect_equal(exemplar2matrix(list(1:2, 3:4)), matrix(1:4, nrow = 2, byrow = TRUE))
  expect_equal(exemplar2matrix(list(1:2)), matrix(1:2, nrow = 1))
  expect_equal(exemplar2matrix(input1.dt[, exemplar]), features)
  expect_equal(exemplar2matrix(input1.dt[3, exemplar]), features[3, , drop = FALSE])
})

test_that("rowMatrix2exemplar produces expected list", {
  expect_equal(rowMatrix2exemplar(matrix(1:2, nrow = 1)), list(1:2))
  expect_equal(rowMatrix2exemplar(matrix(1:2, nrow = 1) %>% data.table), list(1:2))
  expect_equal(rowMatrix2exemplar(1:2), list(1:2))
  expect_equal(rowMatrix2exemplar(features[3, , drop = FALSE]), input1.dt[3, exemplar])
})
...
agent <- list()
agent$cache <- data.table(name = c("FPCA", "MSE"), value = list(), valid = FALSE)
params <- list(
  lambdaFPCA = 1e-08,
  varCutoffFPCA = .995
)
...
test_that("compute_fpca produces expected results", {
  ...
  expect_equal(features %>% ncol, 3) # with lambdaFPCA = 1e-08, varCutoffFPCA = .995
  expect_identical(features[, 1], y_pcafd$scores[, 1])
  expect_identical(agent$cache[name == "FPCA", value][[1]], y_pcafd)
  expect_identical(features, y_pcafd$scores[, 1:3])
  ...
  expect_is(features, "matrix")
  expect_identical(features %>% nrow, y_pcafd$scores %>% nrow)
  expect_equal(features %>% ncol, 6) # with lambdaFPCA = 1e-08, varCutoffFPCA = .995
  expect_identical(features[, 1], yz_pcafd$scores[, 1])
  expect_identical(agent$cache[name == "FPCA", value][[1]], yz_pcafd)
  expect_identical(features, yz_pcafd$scores[, 1:6])
  expect_equal(agent$cache[name == "MSE", value][[1]] %>% length, yz_fd$coefs %>% dim %>% .[2])
})

agent <- list()
agent$cache <- data.table(name = c("FPCA", "MSE"),
                          value = c(list(y_pcafd), list(readRDS("ex4_MSE.rds"))),
                          valid = TRUE)
